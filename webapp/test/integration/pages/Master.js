				...
				actions : {
					...
					iRememberAnIdOfAnObjectThatsNotInTheList : function () {
						return this.waitFor(this.createAWaitForAnEntitySet({
							entitySet : "personal_settingsSet",
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Userid") === oObject.Userid;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Userid;
										} else {
											sCurrentId = aItemsNotInTheList[0].Userid;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("personal_settingsSet", {
											Userid : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							title: oBindingContext.getProperty("Userid")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "personal_settingsSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'personal_settingsSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("Userid");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all personal_settingsSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "personal_settingsSet (" + iExpectedLength + ")"}),
									success : function () {
